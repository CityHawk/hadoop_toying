...
task :create do
    def create_node name
        dropletname = "#{name}-#{@env_id}.#{@config_data['other']['domain']}"
    ...
    # create N servers
    nodes = %w(nn-001 dn-001 dn-002 dn-003).map { |i| "#{i}-#{@env_id}.#{@config_data['other']['domain']}" }
    nodes.each do |n|
        create_node n
    end
...
task :clear, [:e_id] do |t, args|
    ...
    puts `knife environment delete #{args.e_id} -y`
